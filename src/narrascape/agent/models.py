...
class SegmentAnalysis(BaseModel):
    ...
    segment_id: int
    emotion: str = Field(..., description="Dominant emotion: calm, tense, sad, hopeful, dramatic, nostalgic, awe")
    intensity: float = Field(0.5, ge=0.0, le=1.0, description="Emotional intensity 0.0-1.0")
    scene_type: str = Field(..., description="Scene setting: indoor, outdoor, abstract, portrait, landscape, urban")
    key_entities: list[str] = Field(default_factory=list, description="Key visual subjects mentioned in text")
    visual_keywords: list[str] = Field(default_factory=list, description="Atmosphere, lighting, weather descriptors")
...
class ReferenceImageChain(BaseModel):
    ...
    reference_urls: list[str] = Field(default_factory=list, description="Reference image URLs for this chain")
    reference_local_paths: list[str] = Field(default_factory=list, description="Local file paths for reference images")
    target_model: str = Field("", description="Target model: seedream_4.6, seedream_4.0, seedance_2.0, etc.")
    usage_mode: str = Field("reference", description="How to use: reference (参考图), first_frame (首帧), last_frame (尾帧), style (风格参考)")
    sample_strength: float = Field(0.5, ge=0.0, le=1.0, description="Reference influence strength for Seedream. 0.0=weak, 1.0=strong. Default 0.5")
    consistency_target: str = Field("face", description="What to keep consistent: face, body, outfit, style, or all")
    description: str = Field("", description="Human-readable description of this reference chain")
    generated_images: list[str] = Field(default_factory=list, description="URLs of generated images in this chain (for video first/last frame)")
...
class ShotDesign(BaseModel):
    ...
    director_vision: str = Field("", description="Layer 1: Creative brief describing the visual world without technical terms. The director's vision in painter's language.")
    cinematic_format: str = Field("", description="Layer 3: Standardized cinematic language re-presentation of director_vision + technical parameters. Industrial shot-list format with precise terms: EXT./INT., SHOT SIZE, LENS, ANGLE, MOVEMENT, LIGHTING, COMPOSITION, COLOR, DEPTH, ATMOSPHERE, MOOD, DURATION. This determines final output quality.")
    image_prompt: str = Field("", description="Layer 2: Distilled AI image generation prompt derived from cinematic_format with character and scene consistency injected")
    ...
    intensity: float = Field(0.5, ge=0.0, le=1.0, description="Emotional intensity 0.0-1.0")
    metadata: dict = Field(default_factory=dict, description="Extended fields: negative_prompt, focal_length, aperture, camera_angle, lighting_scheme, light_sources, composition, color_palette, atmosphere, depth_of_field, consistency_notes, video_readiness, key_entities, director_vision_backup, seedream_model, seedream_sample_strength, seedance_resolution")
    character_refs: list[str] = Field(default_factory=list, description="Character IDs referenced in this shot. Used for consistency anchoring.")
    style_ref: str = Field("", description="Scene style ID referenced by this shot. Used for visual world consistency.")
    reference_chain_ids: list[str] = Field(default_factory=list, description="ReferenceImageChain IDs used by this shot. Enables multi-reference generation.")
    seedream_model: str = Field("", description="Recommended Seedream model: jimeng-5.0 (default), jimeng-4.6 (better face consistency), jimeng-4.0 (multi-reference)")
    seedance_model: str = Field("", description="Recommended Seedance model: jimeng-video-seedance-2.0 (full features), jimeng-video-seedance-2.0-fast (cost-effective)")
...
class CharacterProfile(BaseModel):
    ...
    """
    char_id: str = Field(..., description="Unique character identifier (e.g., 'char_001', 'protagonist')")
    name: str = Field("", description="Character name if known")
    identity_block: str = Field(..., description="FIXED identity description. Concrete physical traits that NEVER change across shots. This is the 'DNA' of the character.")
    face_description: str = Field("", description="Specific facial features: face shape, skin tone, eye shape/color, nose type, jawline, cheekbones, age cues")
    hair_description: str = Field("", description="Specific hair traits: color, length, texture, style, parting, any distinctive features")
    body_description: str = Field("", description="Body type, height, build, proportions, posture language")
    default_outfit: str = Field("", description="Default clothing and accessories. Can be overridden per scene via outfit_override")
    signature_accessories: list[str] = Field(default_factory=list, description="Signature items that always appear (e.g., 'gold ring on left hand', 'scar on right cheek')")
    outfit_override: str = Field("", description="Per-scene outfit change. If empty, uses default_outfit.")
    expression_range: list[str] = Field(default_factory=list, description="Allowed expressions: [neutral, smiling, frowning, determined, etc.]")
    # Seedream-specific optimization
    seedream_model: str = Field("", description="Recommended Seedream model for this character: jimeng-4.6 (face consistency priority), jimeng-4.0 (multi-reference support), jimeng-5.0 (default)")
    seedream_sample_strength: float = Field(0.5, ge=0.0, le=1.0, description="Reference influence strength for this character. Higher = stronger identity preservation. Default 0.5, recommend 0.6-0.8 for critical characters")
    consistency_score: float = Field(0.0, description="Historical consistency score based on feedback (0.0-1.0)")
    # Reference image for pre-production workflow
    reference_image_url: str = Field("", description="URL or path to the character's reference anchor image. Filled by PreProductionStage.")
    negative_anchors: list[str] = Field(default_factory=list, description="Anti-identity: what this character is NOT. Prevents AI from adding wrong traits.")
...
class SceneStyle(BaseModel):
    ...
    """
    style_id: str = Field(..., description="Unique style identifier (e.g., 'main_style', 'flashback_style')")
    style_name: str = Field("", description="Human-readable name")
    base_color_temperature: str = Field("", description="Base color temperature: warm tungsten (~3200K), daylight (~5600K), cool (~7000K), or specific Kelvin value")
    color_palette: str = Field("", description="Locked color palette for the film world. E.g., 'warm amber + deep teal + muted earth tones'")
    lighting_signature: str = Field("", description="Consistent lighting approach: e.g., 'natural light from screen left, soft diffused window light, Rembrandt lighting'")
    texture_palette: str = Field("", description="Material texture tendency: e.g., 'rough weathered wood, cracked leather, oxidized metal'")
    atmosphere_signature: str = Field("", description="Atmospheric baseline: e.g., 'pervasive dust motes in light beams, hazy distance, golden hour glow'")
    depth_signature: str = Field("", description="Depth of field tendency: e.g., 'shallow DOF for intimacy, deep for landscapes'")
    lens_signature: str = Field("", description="Preferred lens characteristics: e.g., '24mm for wide establishing, 85mm for portraits'")
    style_references: list[str] = Field(default_factory=list, description="Film style references: e.g., 'Roger Deakins', 'Dune (2021)', 'Barry Jenkins'")
    world_rules: list[str] = Field(default_factory=list, description="Narrative world rules: e.g., 'No modern technology', 'Always overcast', 'Warm interior vs cool exterior'")
    consistency_notes: str = Field("", description="Director notes on maintaining visual coherence across the sequence")
    # Seedance-specific optimization
    seedance_model: str = Field("", description="Recommended Seedance model for this style: jimeng-video-seedance-2.0 (full features), jimeng-video-seedance-2.0-fast (cost-effective)")
    seedance_resolution: str = Field("720p", description="Default video resolution for this style: 720p or 1080p")
...
class ConsistencyAnchor(BaseModel):
    ...
    """
    characters: list[CharacterProfile] = Field(default_factory=list, description="All characters in the sequence")
    scene_style: SceneStyle = Field(default_factory=lambda: SceneStyle(style_id="default"), description="The visual world style")
    consistency_rules: list[str] = Field(default_factory=list, description="Rules extracted from sequence analysis")
    drift_warnings: list[str] = Field(default_factory=list, description="Warnings from consistency checks")
    character_id_map: dict[str, str] = Field(default_factory=dict, description="Maps entity names to char_ids")
...
class DesignReport(BaseModel):
    ...
    characters: list[CharacterProfile] = Field(default_factory=list, description="Character profiles for consistency across shots")
    scene_style: SceneStyle | None = Field(None, description="Visual world style guide for the sequence")
    consistency_anchor: ConsistencyAnchor | None = Field(None, description="Cross-shot consistency anchor")
    reference_image_chains: list[ReferenceImageChain] = Field(default_factory=list, description="Reference image chains for Seedream/Seedance multi-reference workflow")
    style_anchor_path: str = Field("", description="Path to the unified style anchor image for style consistency across all shots")
                ...
                for char_ref in seg.character_refs:
                    for char in self.characters:
                        if char.char_id == char_ref:
                            if char.reference_image_url and char.reference_image_url not in ref_images:
    ...
    def to_design_report(self) -> dict:
        ...
        report = {
            ...
            "segments": [
                {
                    ...
                    "negative_prompt": seg.metadata.get("negative_prompt", "") if seg.metadata else "",
                    ...
                    "lighting_scheme": seg.metadata.get("lighting_scheme", "") if seg.metadata else "",
                    ...
                    "depth_of_field": seg.metadata.get("depth_of_field", "") if seg.metadata else "",
                    "consistency_notes": seg.metadata.get("consistency_notes", "") if seg.metadata else "",
                    "video_readiness": seg.metadata.get("video_readiness", "") if seg.metadata else "",
            ...
            "scene_style": {
                ...
                "base_color_temperature": self.scene_style.base_color_temperature if self.scene_style else "",
                "color_palette": self.scene_style.color_palette if self.scene_style else "",
                "lighting_signature": self.scene_style.lighting_signature if self.scene_style else "",
                "texture_palette": self.scene_style.texture_palette if self.scene_style else "",
                "atmosphere_signature": self.scene_style.atmosphere_signature if self.scene_style else "",
                "style_references": self.scene_style.style_references if self.scene_style else [],
                ...
                "seedance_resolution": self.scene_style.seedance_resolution if self.scene_style else "",
            } if self.scene_style else None,
...
class CharacterReferenceImage(BaseModel):
    ...
    image_type: str = Field(..., description="Type: anchor (全身锚点图), turn_front, turn_side, turn_back, expression_neutral, expression_happy, expression_sad, expression_angry, dynamic_pose, outfit_detail")
...
class CharacterReferenceSheet(BaseModel):
    ...
    anchor_image: CharacterReferenceImage | None = Field(None, description="正面全身锚点图 — the master reference")
    turn_images: list[CharacterReferenceImage] = Field(default_factory=list, description="转面图: front, side, back")
    expression_images: list[CharacterReferenceImage] = Field(default_factory=list, description="表情图: neutral, happy, sad, angry, etc.")
    dynamic_images: list[CharacterReferenceImage] = Field(default_factory=list, description="动态图: key poses showing character personality")
    outfit_images: list[CharacterReferenceImage] = Field(default_factory=list, description="服装道具图")
    # Convenience: primary reference for downstream use
    primary_reference_path: str = Field("", description="Path to the anchor image (or first available reference)")
    seedream_model: str = Field("", description="Recommended model for this character")
    seedream_sample_strength: float = Field(0.5, ge=0.0, le=1.0, description="Default sample strength for this character")
...
class EnvironmentReferenceImage(BaseModel):
    ...
    image_type: str = Field(..., description="Type: mood (氛围图), landmark (地标), interior, exterior, detail")
...
class EnvironmentReference(BaseModel):
    ...
    mood_images: list[EnvironmentReferenceImage] = Field(default_factory=list, description="氛围图: lighting, color, atmosphere")
    landmark_images: list[EnvironmentReferenceImage] = Field(default_factory=list, description="地标图: key landmarks or set pieces")
    detail_images: list[EnvironmentReferenceImage] = Field(default_factory=list, description="细节图: textures, props, architectural details")
...
class StoryboardFrame(BaseModel):
    ...
    description: str = Field("", description="Visual description: composition, character positions, action")
    shot_type: str = Field("", description="Recommended shot type: wide, medium, close-up, etc.")
    camera_movement: str = Field("", description="Camera movement: static, pan, tilt, dolly, zoom, etc.")
    camera_angle: str = Field("", description="Camera angle: eye-level, low-angle, high-angle, overhead, etc.")
    character_positions: list[str] = Field(default_factory=list, description="Where each character is positioned in frame")
    emotion: str = Field("", description="Frame emotion")
    duration_hint: float = Field(3.0, description="Suggested duration in seconds")
    character_refs: list[str] = Field(default_factory=list, description="Character IDs present in this frame")
    scene_ref: str = Field("", description="Scene ID referenced")
    reference_image_ids: list[str] = Field(default_factory=list, description="IDs of reference images to use for this frame")
...
class Storyboard(BaseModel):
    ...
    project_title: str = ""
    frames: list[StoryboardFrame] = Field(default_factory=list, description="All storyboard frames in sequence")
...
class PreProductionReport(BaseModel):
    ...
    project_title: str = ""
    style_template: str = ""
    characters: list[CharacterReferenceSheet] = Field(default_factory=list, description="Character reference sheets")
    environments: list[EnvironmentReference] = Field(default_factory=list, description="Scene environment references")
    storyboard: Storyboard = Field(default_factory=lambda: Storyboard(), description="Complete storyboard")
    # Convenience exports
    def to_character_refs_dict(self) -> dict:
        """Export character reference paths for DesignStage consumption."""
        return {
            c.char_id: c.primary_reference_path
            for c in self.characters
            if c.primary_reference_path
        }

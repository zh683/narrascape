...
4. Write `generation.video_prompt`, `generation.negative_prompt`, `generation.duration`, and `generation.motion` so `generate_video` has an executable prompt contract.
5. Write `storyboard_binding.storyboard_frame_ids`, `character_positions`, `scene_ref`, `wardrobe_lock`, `composition_requirements`, and `reference_image_ids` when storyboard frames are available.
6. Write `qa.must_show` and `qa.must_not_show` so `visual_semantic_qa` can review the same contract that guided generation.
